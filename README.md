<!-- ====================== HEADER ====================== -->
<div align="center">

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=0,2,2,5,30&height=210&section=header&text=Henri%20Mafra&fontSize=64&fontColor=ffffff&animation=fadeIn&fontAlignY=34&desc=Ci%C3%AAncia%20de%20Dados%20%E2%80%A2%20Machine%20Learning%20%E2%80%A2%20Full-Stack&descAlignY=56&descSize=18" />

<a href="https://github.com/HenriMafra">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&duration=3000&pause=900&color=7AA2F7&center=true&vCenter=true&width=620&lines=Estudante+de+Ci%C3%AAncia+de+Dados+%26+ML+%F0%9F%A4%96;Construindo+SaaS+com+Next.js+%2B+Supabase+%E2%9A%A1;Apaixonado+por+IA%2C+LLMs+e+Automa%C3%A7%C3%A3o+%F0%9F%A7%A0;Sempre+aprendendo+algo+novo+%F0%9F%9A%80" alt="typing" />
</a>

<br/>

![Profile Views](https://komarev.com/ghpvc/?username=HenriMafra&color=7AA2F7&style=for-the-badge&label=VISUALIZA%C3%87%C3%95ES)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/henri-felipe-marques-mafra-961538352/)
[![Portfólio](https://img.shields.io/badge/Portf%C3%B3lio-FF5722?style=for-the-badge&logo=todoist&logoColor=white)](https://henrimafra.github.io/)

</div>

<!-- ====================== SOBRE ====================== -->
## 🧑‍💻 Sobre mim

```python
class Henri:
    def __init__(self):
        self.role        = "Estudante de Ciência de Dados & ML"
        self.stack_dados = ["Python", "Pandas", "NumPy", "scikit-learn", "Plotly", "Streamlit"]
        self.stack_web   = ["Next.js", "React", "TypeScript", "Supabase", "Cloudflare", "Tailwind"]
        self.foco        = ["IA & LLMs", "Engenharia de Dados", "Automação", "SaaS"]
        self.aprendendo  = "um pouco mais, todo dia 🚀"
```

<br/>

<!-- ====================== STACK ====================== -->
## 🛠️ Tecnologias & Ferramentas

<div align="center">

**Dados & Machine Learning**

<img src="https://skillicons.dev/icons?i=python,tensorflow,pytorch,sklearn,anaconda,mysql,postgres&perline=7" />

**Web & Cloud**

<img src="https://skillicons.dev/icons?i=js,ts,react,nextjs,nodejs,tailwind,supabase,cloudflare,git,github,vscode,figma&perline=12" />

</div>

<br/>

<!-- ====================== ESTATÍSTICAS ====================== -->
## 📊 Estatísticas do GitHub

<div align="center">

<img height="172em" src="https://github-readme-stats-sigma-five.vercel.app/api?username=HenriMafra&show_icons=true&include_all_commits=true&count_private=true&theme=tokyonight&hide_border=true&rank_icon=percentile&title_color=7aa2f7&icon_color=bb9af7" />
<img height="172em" src="https://github-readme-stats-sigma-five.vercel.app/api/top-langs/?username=HenriMafra&layout=compact&langs_count=8&theme=tokyonight&hide_border=true&title_color=7aa2f7" />

<br/>

<img src="https://streak-stats.demolab.com?user=HenriMafra&theme=tokyonight&hide_border=true&ring=7aa2f7&fire=bb9af7&currStreakLabel=7aa2f7" />

<br/>

<img width="85%" src="https://github-readme-activity-graph.vercel.app/graph?username=HenriMafra&theme=tokyo-night&hide_border=true&area=true&color=7aa2f7&line=bb9af7&point=ffffff" />

</div>

<br/>

<!-- ====================== CONTRIBUIÇÕES (PAC-MAN) ====================== -->
## 👾 Minhas contribuições (modo Pac-Man)

<div align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/HenriMafra/HenriMafra/output/pacman-contribution-graph-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/HenriMafra/HenriMafra/output/pacman-contribution-graph.svg">
    <img alt="pacman contribution graph" src="https://raw.githubusercontent.com/HenriMafra/HenriMafra/output/pacman-contribution-graph.svg">
  </picture>
</div>

<br/>

<!-- ====================== JOGO ====================== -->
<div align="center">

<img width="100%" src="https://capsule-render.vercel.app/api?type=rect&color=gradient&customColorList=0,2,2,5,30&height=3" />

## 🎮 Mini-game: Neon Snake

Cansou de só ler estatística? **Joga um Snake** que eu construí — roda 100% no navegador, com placar, recorde salvo e controle por toque no celular. 🐍

<a href="https://henrimafra.github.io/game/">
  <img src="https://img.shields.io/badge/%E2%96%B6_JOGAR_AGORA-9ECE6A?style=for-the-badge&logo=gamejolt&logoColor=000000&labelColor=7dcfff" height="46" />
</a>

<sub>🎯 Setas / WASD para mover • Espaço para pausar • arraste a tela no celular</sub>

<img width="100%" src="https://capsule-render.vercel.app/api?type=rect&color=gradient&customColorList=0,2,2,5,30&height=3" />

</div>

<br/>

<!-- ====================== FOOTER ====================== -->
<div align="center">

⭐️ De [HenriMafra](https://github.com/HenriMafra) — *"a melhor forma de prever o futuro é construí-lo."*

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=0,2,2,5,30&height=120&section=footer" />

</div>
